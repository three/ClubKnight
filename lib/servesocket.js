...
function handleClientConnect(socket) {
    ...
    log.info("Recieved Connection, in waitroom @"+waitid);
    ...
    // connect: "login", which sends their login information
    socket.on("login", (clientdata) => {
        ...
        if (data === null) {
            removeWaitroom(waitid, "Bad data during login.");
            return;
        }
        if (waitroom[waitid].stage !== WAITROOM_STAGE_JOIN) {
            removeWaitroom(waitid, "You already tried to login!");
            return;
        }

        log.info("Attempt to login with token: ", data.token);
        ...
        persist.getAuth({
            ...
            if (!row) {
                removeWaitroom(waitid, "Bad token!");
                return;
            }
    ...
    socket.on("disconnect", () => {
        log.info("Player @"+waitid+" disconnected");
        waitroom[waitid].stage = WAITROOM_STAGE_INACTIVE;
    });
}

function removeWaitroom(waitid, message) {
    log.info("Removing @"+waitid+" from waitroom, because ", message);
    waitroom[waitid].socket.emit("remove", {
        time: Date.now(),
        message: message,
    });
    waitroom[waitid].socket.disconnect();

    waitroom[waitid].socket = null;
    waitroom[waitid].netid = "";
    waitroom[waitid].stage = WAITROOM_STAGE_INACTIVE;
    waitroom[waitid].enterTime = -1;
}
    ...
    for (let i=0;i<waitroom.length;i++) {
        //if (openSpots>0) {
            if (waitroom[i].stage === WAITROOM_STAGE_READY) {
                ...
                waitroom[i].stage = WAITROOM_STAGE_INACTIVE;
                //openSpots--;
            }
        //}

        if (waitroom[i].stage !== WAITROOM_STAGE_INACTIVE &&
            waitroom[i].enterTime < now-1000*60) {
            log.info("Removing straggler @"+i);
            removeWaitroom(i, "Straggler");
        }
    }
